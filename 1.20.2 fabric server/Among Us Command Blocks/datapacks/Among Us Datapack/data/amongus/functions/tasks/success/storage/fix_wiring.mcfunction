function amongus:tasks/reset/storage/fix_wiring
...
tag @s remove storage_fix_wiring
...
execute as @s[scores={wiring_tasks_left=1..}] if score Manager randomNumber matches 3 run tag @s add security_fix_wiring
execute as @s[scores={wiring_tasks_left=1..}] if score Manager randomNumber matches 3 run tellraw @s {"text":"The next part of this task takes place in: Security, Fix Wiring","color": "green"}

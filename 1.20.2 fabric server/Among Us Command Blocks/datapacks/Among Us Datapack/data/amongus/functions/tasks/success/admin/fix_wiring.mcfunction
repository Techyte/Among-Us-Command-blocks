function amongus:tasks/reset/admin/fix_wiring
...
execute as @s[scores={wiring_tasks_left=1..}] if score Manager randomNumber matches 4 run tag @s add navigation_fix_wiring
execute as @s[scores={wiring_tasks_left=1..}] if score Manager randomNumber matches 4 run tellraw @s {"text":"The next part of this task takes place in: Navigation, Fix Wiring","color": "green"}